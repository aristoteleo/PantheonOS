...
class TestRoundTrip:
    def test_a_saved_store_reloads_with_its_individuals_and_measurements(self, tmp_path):
        ...
        save_run(str(tmp_path), store, AgentMapElites())
...
class TestResume:
    def test_resuming_keeps_the_earlier_individuals_and_adds_to_them(self, tmp_path):
        ...
        m1 = AgentMapElites(num_islands=1, function_weight=1.0, llm_weight=0.0)
        first = run(m1, tmp_path, budget=4)
        assert len(first.store) == 5          # seed + 4

        m2 = AgentMapElites(num_islands=1, function_weight=1.0, llm_weight=0.0)
    ...
    def test_resuming_with_an_already_spent_budget_does_nothing(self, tmp_path):
        m1 = AgentMapElites(num_islands=1)
        run(m1, tmp_path, budget=4)
        res = run(AgentMapElites(num_islands=1), tmp_path, budget=4, resume=True)
    ...
    def test_resuming_does_not_re_evaluate_the_seed(self, tmp_path):
        ...
        run(AgentMapElites(num_islands=1), tmp_path, budget=2)
        ...
        asyncio.run(evolve(
            method=AgentMapElites(num_islands=1), variator=Counter(),
    ...
    def test_the_method_state_survives_and_reconciles(self, tmp_path):
        m1 = AgentMapElites(num_islands=2, function_weight=1.0, llm_weight=0.0)
        run(m1, tmp_path, budget=4)
        elites_before, islands_before = dict(m1.elites), dict(m1.island_of)

        m2 = AgentMapElites(num_islands=2, function_weight=1.0, llm_weight=0.0)
    ...
    def test_a_method_can_drop_its_state_and_rebuild_from_the_store(self, tmp_path):
        ...
        run(AgentMapElites(num_islands=1, feature_bins=4), tmp_path, budget=4)
        store, _, _ = load_run(str(tmp_path))

        fresh = AgentMapElites(num_islands=1, feature_bins=16)     # different grid
...
class TestReporting:
    ...
    def test_a_new_format_run_renders_an_html_report(self, tmp_path):
        ...
        m = AgentMapElites(num_islands=1, function_weight=1.0, llm_weight=0.0)
