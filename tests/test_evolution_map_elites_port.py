...
the same properties of `MapElitesIslands`, which is the port. Where the two must agree, they are
...
from pantheon.evolution.methods import MapElitesIslands
...
def method(**kw) -> MapElitesIslands:
...
def place(m: MapElitesIslands, ctx: EvolveContext, i: Individual, island=0) -> bool:
...
class TestFitness:
    ...
    def test_fitness_matches_the_originals_number(self):
        ...
        m = MapElitesIslands(num_islands=1, feature_bins=4, function_weight=0.8, llm_weight=0.2)
        ctx = ctx_with(Store())
