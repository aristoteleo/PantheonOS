...
3. Inspect each operation and instance. On missing acknowledgements, keep the
   outcome unknown and inspect/retry the same operation ID; never create another
...
The first successful preparation migrates the **on-disk ledger** from version 1
to 2. Older Runners reject it instead of mistaking a process-free reservation for
dead state. Existing nodes that never use preparations stay on ledger version 1.
