"""Evaluator for DIRECT gene-panel evolution (Pantheon Evolution).

The evolving artifact is a GENE PANEL, not code: the workspace holds ``panel.txt`` — one gene
symbol per line, drawn from the dataset's gene universe. Fitness is the panel's quality on
panel-selection-bench (Janesick breast-cancer scRNA), scored REMOTELY on Modal (the data + the
cached full-transcriptome ceiling live there), so this evaluator is a thin client.

``quality_score`` = mean over dimensions {1 identifiability, 3 structure, 4 reconstruction} of
the per-dimension ``relative`` (panel score / full-transcriptome ceiling). Higher is better.
A panel is INVALID (score 0) only if it has < 2 genes in the universe.

Returned metrics feed the mutation agent: it sees quality_score + the per-dimension breakdown
(so it knows which dimension is weak — reconstruction/dim4 is the usual bottleneck).
"""
import os
import time
from typing import Any, Dict

SCORE_APP = "panelbench-evo-score"
