...
def run_single_case(args):
    ...
    try:
        ...
        # Build run command:
        #   timeout {ceil+1.5} bash -c 'prlimit --as=3221225472 --cpu={ceil+0.1} /usr/bin/time -f "..." ./a.out < input > output; sync -f {output_file}'
        ...
        cmd = (
            ...
            f"bash -c 'prlimit --as=3221225472 --cpu={time_limit_ceil + 0.1} "
        ...
        # The SimpleTES-cache testers print "Score = N" on stderr; the official AHC `vis` tools
        # print it on stdout (and complaints on stderr). Take the last score line from either.
        lines = stderr.splitlines() + (judge.stdout or "").strip().splitlines()
        if not lines:
            return {"case_idx": case_idx, "score": 0, "judge": "WA",
                    "msg": "Empty tester output", "time": elapsed}

        m = None
        for ln in reversed(lines):
            m = re.match(r"Score = (\d+)", ln.strip())
            if m:
                break
        if m is None:
            return {"case_idx": case_idx, "score": 0, "judge": "WA",
