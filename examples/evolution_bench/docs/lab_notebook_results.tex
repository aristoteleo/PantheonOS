...
\begin{table}[H]\centering\small
\caption{Ablations on hexagon-11 and Heilbronn convex-13, all 24 arms finished (three seeds each, AlphaEvolve-suite budgets, deepseek-v4-flash-0731). Best per arm (s0 / s1 / s2) and mean; higher is better. The full-method and port rows are the AlphaEvolve-campaign arms.}
\label{tab:ablc}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
its mean (0.839) is below the full method and the port.}
\label{fig:ablc}
\end{figure}



\paragraph{Two more AHC tasks: AHC058 and AHC046.} To test whether the AHC039 pattern
(Table~\ref{tab:part2}) holds on other AtCoder heuristic contests, the same 21-arm block (same
configuration, budgets and one eval server per wave) was run on Sherlock on AHC058 (Apple
Incremental Game) and AHC046 (Skating with Blocks), both from ALE-Bench with their official
generators and scorers, 150 public cases each. Table~\ref{tab:abld} and Figure~\ref{fig:abld} carry
the results. The AHC039 pattern does not repeat. On AHC058 our SimpleTES port has the best mean
(5.327) with all three seeds above 5.28, while the full method has one weak seed (5.159) next to
the single best arm of the block (5.371); on AHC046 the no-ideas variant leads (1.451, three seeds
within 0.006) with the full method second (1.430). The spread of the seven method means is 1.4\% on
AHC058 and 2.3\% on AHC046, while one method's three seeds span 4\% (LabNotebook on AHC058) to 6\%
(the port on AHC046): three seeds cannot separate any two methods on these tasks. Two things do
survive the noise. First, the items column: under the shared 480-call budget the full method
evaluates about 26\% fewer candidates than the port (175 against 238 on AHC058, 181 against 290 on
AHC046), because its Understand, Propose, Digest and Update calls are booked against the same
budget. On these incremental-tuning tasks, where the seed heuristic is already strong and the gains
come from many small local edits, the number of evaluated edits matters more than their diversity.
Second, no ablation moves the score by more than the seed noise: on these two tasks neither the
ideas nor the notebook nor the digest has a measurable effect.

\begin{table}[H]\centering\small
\caption{The AHC039 block repeated on AHC058 and AHC046, all 42 arms finished (three seeds each,
deepseek-v4.1-flash without thinking, 480-call budgets, best per arm and mean, higher is better).
\emph{items} = mean number of candidates the arm evaluated within the budget. Seed programs re-read at
4.8675 (AHC058) and 1.1266 (AHC046) on every server.}
\label{tab:abld}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
\caption{The ablation block on AHC058 (left) and AHC046 (right), final. Unlike AHC039
(Table~\ref{tab:part2}) and the two geometry tasks (Figure~\ref{fig:ablc}), no variant separates from the others: the port leads
AHC058 on the mean, the no-ideas variant leads AHC046, and every group's three seeds overlap every
other group's. The full method's AHC058 seed 1 (5.159) is the lowest arm of that block and its seed 2
(5.371) the highest.}
\label{fig:abld}
\end{figure}

\paragraph{What the ablations remove, and what they do not.} Across the five tasks with a full
ablation block (AHC039, hexagon-11, Heilbronn-13, AHC058, AHC046) the only two changes with effects
...
both are null on AHC058 and AHC046. Removing the notebook or the digest is within noise on four tasks
...
second question (ideas proposed from the problem and the parent program alone, \texttt{--ln-no-history}
with both other switches off) is running on hexagon-11 and Heilbronn-13 and will be added here.
...
``clear'' means the ranges do not overlap. LabNotebook is at least as good on 17 of 21
...
leaning on C2, Heilbronn-13, sums vs differences and AHC046. It loses clearly on Hadamard-29 and leans
behind on C3, kissing-11 and AHC058, the one AHC task where the port is ahead. Ten pairs are ties,
...
without the score advantage on AHC058.}
\label{tab:h2hcost}
...
\item \textbf{Where the idea loop pays: the open-ended task.} On AHC039 all six LabNotebook arms
  ...
  changes what gets explored. It is not every AtCoder task: on AHC058 and AHC046 (Table~\ref{tab:abld})
  the same block gives the port the lead on one and the no-ideas variant on the other, all within
  seed noise.
...
\item \textbf{What the ablations attribute the effect to.} On the five tasks with a full block
  ...
  prose notebook and the digest are within noise on four tasks and hurt on hexagon-11. On AHC058 and
  AHC046 nothing separates: the port leads one, the no-ideas variant the other, and the full method
  evaluates about a quarter fewer candidates than the port under the shared call budget.
\item \textbf{Limits of the evidence.} Three arms per cell, model sampling unseeded, and the
  no-notebook arm still shows the proposer the record of tried ideas; the arm that removes all
  memory is running (see the paragraph after Table~\ref{tab:abld}).
\end{itemize}
