...
\begin{table}[H]\centering\small
\caption{Part 1 snapshot (2026-09-09 23:35 PDT): best per arm (s0 / s1 / s2) and mean. Dashes are the two
sums-vs-differences port-mirror engine arms still running on Sherlock; the upstream engine keeps no
partial results.}
\label{tab:part1}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
upstream, port-mirror & 1.0692 & --- & --- & 1.0692 \\
...
\begin{table}[H]\centering\small
\caption{Part 2 (2026-09-09 23:35 PDT). $C_2$ and sums vs differences: all 24 ablation arms finished. AHC039: the
Modal half-block, frozen where the credit ran out ($^\circ$, 35--155 rows of $\sim$180, old merge);
the Sherlock re-run on the refined merge is in progress and replaces these rows when done.}
\label{tab:part2}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\multicolumn{5}{@{}l}{\textbf{$C_2$} (higher is better)} \\
...
above the full method (1.076--1.085 against 1.0718). On these two tasks, then, the ideas carry the
effect and the notebook and digest do not show a measurable benefit; AHC039, where the notebook has
the most to remember, is the open question the Sherlock block answers.}
\end{figure}
