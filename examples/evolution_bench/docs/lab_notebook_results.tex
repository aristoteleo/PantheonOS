...
\begin{table}[H]\centering\small
\caption{Part 2 (2026-09-10 09:15 PDT). $C_2$ and sums vs differences: all 24 ablation arms finished. AHC039,
two datasets: the Modal half-block, frozen where the credit ran out ($^\circ$, 35--155 rows of
$\sim$180, old merge), and the Sherlock run on the refined merge, 11~h in ($^\circ$, best so far).
In the Sherlock run the port and no-ideas rows are not informative: a regression in the refined
merge starved those arms of usable replies (Section~\ref{sec:merge}); the corrected merge is in
the pending rerun. The upstream engine keeps no partial results.}
\label{tab:part2}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\multicolumn{5}{@{}l}{\textbf{$C_2$} (higher is better)} \\
...
no ideas & 0.9064 & 0.9096 & 0.9103 & 0.9087 \\
one long trajectory & 0.9266 & 0.9244 & 0.9172 & 0.9227 \\
no notebook & 0.9048 & 0.9108 & 0.9251 & 0.9136 \\
no digest & 0.9036 & 0.9304 & 0.9273 & 0.9204 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{sums vs differences} (higher is better)} \\
\midrule
LabNotebook (full) & 1.0668 & 1.0865 & 1.0621 & 1.0718 \\
no ideas & 1.0668 & 1.0754 & 1.0865 & 1.0762 \\
one long trajectory & 1.0827 & 1.0960 & 1.0763 & 1.0850 \\
no notebook & 1.0674 & 1.0754 & 1.0927 & 1.0785 \\
no digest & 1.0947 & 1.0674 & 1.0668 & 1.0763 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{AHC039, Modal half-block} (official points; $^\circ$ = frozen partial)} \\
\midrule
upstream SimpleTES & --- & 3742 & --- & 3742 \\
our port & 3677$^\circ$ & 3524$^\circ$ & 3702$^\circ$ & 3634 \\
LabNotebook (full) & 3720$^\circ$ & 3757$^\circ$ & 3727$^\circ$ & 3735 \\
no ideas & 3395$^\circ$ & 3661$^\circ$ & --- & 3528 \\
one long trajectory & 3573$^\circ$ & 3700$^\circ$ & 3722$^\circ$ & 3665 \\
no notebook & 3708$^\circ$ & 3713$^\circ$ & 3737$^\circ$ & 3718 \\
no digest & 3702$^\circ$ & 3752$^\circ$ & 3700$^\circ$ & 3718 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{AHC039, Sherlock run, refined merge, 11 h in} (official points; $^\circ$ = running)} \\
\midrule
upstream SimpleTES & --- & --- & --- & --- \\
our port (starved) & --- & --- & 3705$^\circ$ & 3705 \\
LabNotebook (full) & 3689$^\circ$ & 3739$^\circ$ & 3716$^\circ$ & 3715 \\
no ideas (starved) & --- & 3658$^\circ$ & 3697$^\circ$ & 3677 \\
one long trajectory & 3694$^\circ$ & --- & 3714$^\circ$ & 3704 \\
no notebook & 3689$^\circ$ & 3712$^\circ$ & 3747$^\circ$ & 3716 \\
no digest & 3696$^\circ$ & 3716$^\circ$ & 3740$^\circ$ & 3717 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Part 2. Cheap tasks final: removing the ideas is the one variant below the full method on
C2 (0.9087 against 0.9180); removing the digest (0.9204), removing the notebook (0.9136) and a
single long trajectory (0.9227) do not hurt, and on sums vs differences every variant lands at or
above the full method (1.076--1.085 against 1.0718). AHC039 is shown twice: the Modal half-block
(old merge) and the Sherlock run on the refined merge, both partial. In both, the full method,
no-notebook and no-digest arms sit within $\sim$20 points of each other, the seed's own machine
noise, so on the evidence so far the notebook and digest do not separate from the full method on
AHC039 either; the port and no-ideas curves of the Sherlock run are starved (see text) and the
rerun on the corrected merge decides them.}
\end{figure}
...
\label{sec:merge}
\paragraph{What starved the port and no-ideas arms, and the rerun.} The refined merge had a
second defect (it took the last marker pair in the reply, so a closing sentence naming both markers
yielded an empty block; corrected), but that is not what starved the arms. The rejected replies were
captured and every one is empty. The cause is a missing flag: every AHC039 spec, from the flash and
pro campaigns through this block, omitted \texttt{--max-output-tokens}, so our methods ran at their
32\,768-token default while the upstream-engine arms in the same specs and every cheap-task arm ran
at 64\,000. DeepSeek spends about 26k tokens thinking on this task before it writes, so at 32k it
often had nothing left, most of all on prompts without an idea to anchor on (no-ideas arm: 100\% of
replies empty; port: 62\%; the full method $\sim$30\%, against 22--27\% for the same arms on Modal,
where provider routing happened to be kinder). Bounding the reasoning to 16k restored the item rate
but not the quality (75\% of compilable candidates scored below 0.5 against 2\% unbounded), so the
rerun uses the corrected merge, the pinned model id, and a 64\,000-token output limit for every arm,
which is what the engine arms and the cheap tasks always had. The block's AHC039 numbers are therefore
not comparable one-to-one with the flash/pro campaign's, which ran our methods at 32k on the old
merge; within the block every arm shares the same configuration.
