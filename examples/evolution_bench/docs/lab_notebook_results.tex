...
\subsection{Baseline fidelity and ablations}
\label{sec:parts}
...
\paragraph{Four more AHC tasks: AHC058, AHC046, AHC026 and AHC024.} To test whether the AHC039
...
Incremental Game), AHC046 (Skating with Blocks), AHC026 (Stack of Boxes) and AHC024 (Topological
Map), all from ALE-Bench with their official generators and scorers, 150 public cases each.
Table~\ref{tab:abld} and Figure~\ref{fig:abld} carry the results. The four split two ways. On the
first three the AHC039 pattern does not repeat. On
...
AHC024 is the other kind of task, and the AHC039 pattern returns in a sharper form. Its seed (an
identity map with greedy erasure, 0.638) is hard to improve by obvious edits: all nine arms without
ideas (the port, the authors' engine and the no-ideas variant) finish within 0.002 of the seed, the
port and engine arms at the 30-hour wall (192 and 279 candidates) with their best still the seed
program. Of the twelve idea-driven arms, three break through, no-digest to 1.267 and 1.030 and
no-notebook to 1.071 (+60 to +100\%), and the full method's seed 1 reaches 0.707; the full method's
mean (0.664) is above every port seed, but two of its three seeds are within 1\% of the seed
program. A breakthrough is a rare event at this budget, so three seeds cannot rank the LabNotebook
variants against each other; what the block does establish is that every breakthrough came from an
idea-driven arm and none from a SimpleTES-family one.

\begin{table}[H]\centering\small
\caption{The AHC039 block repeated on AHC058, AHC046, AHC026 and AHC024, all 84 arms finished
(three seeds each, deepseek-v4.1-flash without thinking, 480-call budgets, best per arm and mean,
higher is better). \emph{items} = mean number of candidates the arm evaluated within the budget.
Seed programs re-read at 4.8675 (AHC058), 1.1266 (AHC046), 0.8565 (AHC026) and 0.6380 (AHC024) on
every server. The AHC024 port and engine arms stopped at the 30-hour wall before their call budget.}
\label{tab:abld}
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
\midrule
\multicolumn{6}{@{}l}{\textbf{AHC024} (official points, thousands)} \\
\midrule
LabNotebook (full) & 0.6432 & 0.7068 & 0.6430 & 0.6643 & 222 \\
no ideas & 0.6395 & 0.6395 & 0.6395 & 0.6395 & 227 \\
one long trajectory & 0.6412 & 0.6398 & 0.6395 & 0.6402 & 245 \\
no notebook & 1.0708 & 0.7029 & 0.6778 & 0.8172 & 179 \\
no digest & 0.6393 & 1.0302 & \textbf{1.2674} & \textbf{0.9790} & 268 \\
\midrule
our SimpleTES port & 0.6395 & 0.6395 & 0.6395 & 0.6395 & 192 \\
upstream SimpleTES & 0.6389 & 0.6395 & 0.6395 & 0.6393 & 279 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{The ablation block on AHC058 (top left), AHC046 (top right), AHC026 (bottom left) and
AHC024 (bottom right), final. On the
first three, unlike AHC039 (Table~\ref{tab:part2}) and the two geometry tasks
(Figure~\ref{fig:ablc}), no LabNotebook variant separates from the others: the port leads AHC058 on
the mean, the no-ideas variant leads AHC046, the port, no-ideas and the authors' engine tie on
AHC026, and on the first two every group's three seeds overlap every other group's. On AHC026 the
full method's three seeds (0.915--0.921) all sit below the port's (0.924--0.927). AHC024 (bottom right) is
the opposite picture: every port, engine and no-ideas arm sits on the seed line and the three
breakthrough arms (1.27, 1.07, 1.03) are all idea-driven variants.}
\label{fig:abld}
\end{figure}

\paragraph{What the ablations remove, and what they do not.} Across the seven tasks with a full
ablation block (AHC039, hexagon-11, Heilbronn-13, AHC058, AHC046, AHC026, AHC024) the only two changes
with effects beyond seed noise are removing the ideas (hexagon-11 $-$20\%, Heilbronn-13 $-$27\%,
AHC024 $-$4\%, AHC039 $-$1\%) and collapsing the parallel short trajectories into one long one
(Heilbronn-13 $-$8\%, hexagon-11 $-$4\%, AHC024 $-$4\%); both are null on AHC058, AHC046 and AHC026.
Removing the notebook or the digest is within noise on five tasks and \emph{helps} on hexagon-11 and,
by the luck of where the three breakthroughs fell, on AHC024. One caveat on what these two switches test: \texttt{--ln-no-notebook}
...
``clear'' means the ranges do not overlap. LabNotebook is at least as good on 18 of 23
task--campaign pairs and better where the search is open: all three AHC039 campaigns (the only clear,
repeatable gap, +21, +52 and +40 points, every pro LabNotebook arm above every port arm), hexagon-11,
AHC024 (every SimpleTES arm stays on the seed program; clear by the non-overlap rule, though two of the
three LabNotebook seeds are within 1\% of the seed), and leaning on C2, Heilbronn-13, sums vs
differences and AHC046. It loses clearly on Hadamard-29 and
...
v4.1 & AHC024 & \textbf{0.6643} [0.643, 0.707] & 0.6395 [0.639, 0.640] & +3.9\% & LabNotebook, clear \\
\bottomrule
\end{tabular}
\end{table}
...
AHC026 40\% fewer tokens and spend, AHC024 30\% fewer tokens) without the score advantage on AHC058
and AHC026.}
\label{tab:h2hcost}
...
v4.1 & AHC024 & \textbf{6.2} & 8.8 & 482 & 481 & \textbf{1.69} / 2.07 \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Where the idea loop pays: the open-ended task.} On AHC039 all six LabNotebook arms
  ...
  no-ideas variant on the third. AHC024 is the AHC039 kind again: every SimpleTES arm stays on the
  seed program and the only breakthroughs (three of twelve arms, +60 to +100\%) are idea-driven.
...
\item \textbf{What the ablations attribute the effect to.} On the five tasks with a full block
  ...
  On AHC024 the nine arms without ideas all stay at the seed and three of the twelve with ideas break
  through.
