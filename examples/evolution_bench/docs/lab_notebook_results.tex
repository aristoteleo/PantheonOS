...
\begin{table}[H]\centering\small
\caption{Part 2 (2026-09-12 21:30 PDT). $C_2$ and sums vs differences: all 24 ablation arms finished (0731). AHC039:
the final block, all 21 arms finished on Sherlock under one configuration (deepseek-v4.1-flash without
thinking, 32k output limit, block-edit-only merge, three retries, one eval server per wave, equal
480-call budgets); the Modal half-block above it is the earlier, frozen attempt on 0731 with the old
merge and is kept for provenance only. Seed re-reads during the block sat at 3686--3711 points, so
differences below $\sim$20 points are machine noise.}
\label{tab:part2}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\multicolumn{5}{@{}l}{\textbf{$C_2$} (higher is better)} \\
...
\midrule
\multicolumn{5}{@{}l}{\textbf{AHC039, final block} (official points; deepseek-v4.1-flash, no thinking; all 21 arms finished)} \\
\midrule
upstream SimpleTES & 3711 & 3698 & 3652 & 3687 \\
our port & 3691 & 3721 & 3734 & 3715 \\
...
\caption{Part 2, final. Cheap tasks: removing the ideas is the one variant below the full method on
C2 (0.9087 against 0.9180); removing the digest (0.9204), the notebook (0.9136) or running one long
trajectory (0.9227) does not hurt, and on sums vs differences every variant lands at or above the
full method. AHC039 (final block, right): the full method leads at 3755 points with every seed at or
above 3728; removing the ideas drops to 3714, the level of the port (3715), so the ideas carry the
effect here too; removing the notebook (3728) or the digest (3723), or one long trajectory (3722),
each cost about 30 points, at the edge of the $\sim$20-point noise band; the authors' engine trails
at 3687.}
\end{figure}
...
``clear'' means the ranges do not overlap. LabNotebook is at least as good on 15 of 18
task--campaign pairs and better where the search is open: both AHC039 campaigns (the only clear,
repeatable gap, +21 and +52 points, every pro LabNotebook arm above every port arm), hexagon-11, and
leaning on C2, Heilbronn-13 and sums vs differences. It loses clearly on Hadamard-29 and leans
behind on C3 and kissing-11. Ten pairs are ties, mostly because both methods reach the known
optimum (packing, Erd\H{o}s, min--max) or sit inside replicate spread.

\begin{table}[H]\centering\small
\caption{LabNotebook vs our SimpleTES port on every main-campaign task: mean of three seeds
[min, max] of the combined score, higher is better.}
\label{tab:h2h}
