...
\paragraph{The same ablations on the other tasks where LabNotebook leads (in progress).} Hexagon-11
and Heilbronn convex-13 are the two AlphaEvolve-suite tasks on which LabNotebook led the port
(Table~\ref{tab:h2h}). The four ablations were launched there on 2026-09-14 (three seeds each,
AlphaEvolve-suite budgets, deepseek-v4-flash-0731); the full-method and port references are the
AlphaEvolve-campaign arms themselves. Table~\ref{tab:ablc} and Figure~\ref{fig:ablc} carry the
snapshot; $^\circ$ marks arms still running (best so far).

\begin{table}[H]\centering\small
\caption{Ablations on hexagon-11 and Heilbronn convex-13 (snapshot 2026-09-14 15:00 PDT, 3 of 24 arms
finished; $^\circ$ = running). Best per arm (s0 / s1 / s2) and mean; higher is better.}
\label{tab:ablc}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
one long trajectory & 0.4913$^\circ$ & --- & 0.9465$^\circ$ & 0.7189 \\
no notebook & 0.8486$^\circ$ & 0.9803$^\circ$ & 0.9576$^\circ$ & 0.9288 \\
no digest & 0.9776$^\circ$ & 0.9825$^\circ$ & 0.9593$^\circ$ & 0.9732 \\
\midrule
...
no ideas & 0.7344$^\circ$ & 0.5696$^\circ$ & 0.6876$^\circ$ & 0.6639 \\
one long trajectory & 0.5696$^\circ$ & 0.5696$^\circ$ & 0.8249$^\circ$ & 0.6547 \\
no notebook & 0.9296$^\circ$ & 0.9058$^\circ$ & 0.8657$^\circ$ & 0.9004 \\
no digest & 0.8420$^\circ$ & 0.7108$^\circ$ & 0.8740$^\circ$ & 0.8090 \\
\midrule
our SimpleTES port & 0.9158 & 0.8676 & 0.8833 & 0.8889 \\
\bottomrule
\end{tabular}
...
\caption{Ablations on hexagon-11 (left) and Heilbronn convex-13 (right), snapshot. On hexagon-11 the
three no-ideas arms have finished at 0.786 / 0.491 / 0.771, below the full method (0.852) and at or
above the port (0.491, all three seeds): the ideas are what lift LabNotebook above the port there,
while the no-notebook and no-digest arms are already at or above the full method's mean. Heilbronn
is too early to read; the one-long arms are slow because each of their items is an eight-step
trajectory.}
\label{fig:ablc}
\end{figure}
