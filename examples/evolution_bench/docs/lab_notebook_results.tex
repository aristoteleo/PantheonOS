...
\begin{center}\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{34mm}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{58mm}@{}}
\toprule
...
\midrule
seed evaluation & \multicolumn{2}{l}{once (engine default: 16 repeats, max kept)} & \\
candidates & \multicolumn{2}{l}{100 (60 on C1), each one request in the engine's streaming mode} & \\
tokens & \multicolumn{2}{l}{64k output per request; 3.5\,M completion-token cap} & \\
evaluation & \multicolumn{2}{l}{our evaluator, 300 s; AHC039 on the shared eval server} & \\
\bottomrule
\end{tabular}
\end{center}
...
completion tokens only, so its arms consume more total tokens than our 3.5\,M prompt+completion
cap (3.1--6.7\,M on the finished arms); and the engine rejects a reply that drops the
...
\begin{center}\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{26mm}>{\raggedright\arraybackslash}p{50mm}>{\raggedright\arraybackslash}p{30mm}>{\raggedright\arraybackslash}p{46mm}@{}}
\toprule
...
\begin{table}[H]\centering\small
\caption{Part 1 snapshot (2026-09-08 16:20 PDT): best per arm (s0 / s1 / s2) and mean. Dashes are
...
no notebook & 0.9124$^\circ$ & --- & --- & 0.9124 \\
no digest & 0.9098$^\circ$ & --- & --- & 0.9098 \\
\midrule
\multicolumn{5}{@{}l}{\textbf{sums vs differences} (higher is better)} \\
\midrule
...
no ideas & 3693$^\circ$ & 3697$^\circ$ & --- & 3695 \\
one long trajectory & 3689$^\circ$ & 3716$^\circ$ & --- & 3702 \\
...
far. The no-notebook and no-digest arms on the cheap tasks have just started; the AHC039 block,
where the effect to explain is largest, is a few hours in.}
\end{figure}
