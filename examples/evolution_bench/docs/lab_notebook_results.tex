...
\label{sec:merge}
\paragraph{A regression in the refined merge, and the corrected one.} The refined merge searched the
whole reply for the last marker pair. The block prompt asks the model to keep both marker lines,
and the model often closes with a sentence naming them (``both \texttt{EVOLVE-BLOCK-START} and
\texttt{EVOLVE-BLOCK-END} are kept''); that sentence then supplied the last pair, the text between
was empty, and the reply was dropped as having no code block. Dropped replies per accepted item
rose from 0.2--1.8 (Modal, old merge) to 0.6--40 (Sherlock, refined merge), worst on the port and
the no-ideas variant, whose arms produced one to eleven items in eleven hours. The corrected merge
looks for the pair inside the fenced code first, then on the fence-less reply, and skips a pair with
nothing between it; it keeps the whole-file and nested-fence handling, and rejected replies now log
their shape. The Sherlock run in Table~\ref{tab:part2} is the regressed one; its full-method,
no-notebook, no-digest and engine arms are usable, its port and no-ideas arms are not, and a rerun
of the whole block on the corrected merge follows.
