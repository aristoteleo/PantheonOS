...
\multicolumn{6}{@{}l}{\footnotesize published: AlphaEvolve 1.4556 $\cdot$ TTT-Discover 1.454555 $\cdot$ SimpleTES 1.453675 (record);
  $^\ddagger$ reproduces the record value} \\
\midrule
...
\item \textbf{AlphaEvolve suite.} Batch A: packing $n{=}32$ LabNotebook on the record 3/3, SimpleTES
  ...
  (0.885 vs 0.817). Across the six tasks so far: LabNotebook ahead on three, SimpleTES on one, two
  ties --- and both far ahead of the two agent-based methods. All are behind the 2026 systems by
  margins their extra compute explains.
