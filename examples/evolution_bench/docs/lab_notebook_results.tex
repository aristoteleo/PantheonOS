...
captured and every one is empty: the provider caps completion at 32\,768 tokens whatever we ask,
and for prompts without an idea to anchor on the model spends the whole budget reasoning and returns
no content (no-ideas arm: 100\% of replies empty; port: 62\%; the full method $\sim$30\%, against
22--27\% for the same arms on Modal two days earlier). The model version was unchanged (the floating
alias still resolved to \texttt{deepseek-v4-flash-0731}); the routing between providers is not
ours to fix. The rerun therefore pins the model id, bounds reasoning to 16k tokens on all six of
our arms (the upstream engine has no such knob and copes by rejecting empty replies), and runs on
the corrected merge; a two-arm smoke of the worst cases produced 3--4 items in 13 minutes where the
starved run managed 1--6 in ten hours. The reasoning bound is a change of LLM configuration relative
to the flash/pro campaigns and is stated as such; it is identical for every arm of the block.
