---
id: spatial3d
name: 3D Spatial Transcriptomics — on the desktop
description: |
  Open and drive a 3D point-cloud viewer for single-cell spatial data on the desktop — every cell a point at its (x,y,z), coloured by a
  ...
  cells. app="spatial3d".
tags: [visualization, spatial, transcriptomics, 3d, single-cell, anndata, h5ad, stereo-seq, merfish]
---
...
```python
desktop_update(window_id, {"colorBy": "gene", "gene": "Sox2", "colormap": "plasma"})
desktop_update(window_id, {"threshold": 1.5})           # hide cells below this expression
desktop_update(window_id, {"colorBy": "cluster", "cluster": "neural tube"})  # focus one cluster
desktop_update(window_id, {"mode": "umap"})             # 3d | 2d | umap
desktop_update(window_id, {"pointSize": 1.6, "opacity": 0.8})
desktop_update(window_id, {"camera": {"rotationOrbit": 120, "rotationX": 25, "zoom": 4}})
desktop_screenshot(window_id)                            # reads the deck.gl WebGL canvas
```
...
This viewer is installed as the desktop app `spatial3d` ("Spatial 3D"). Everything above drives it through the `desktop` tools, which reach every window of it — including ones the USER opened. What follows is what is specific to this app.
...
- **Open by state**: `desktop_open(app="spatial3d", state={...})` with the state contract documented above.
- **Force this viewer** for a file another app also claims: `desktop_open(app="spatial3d", path=...)`. `desktop_apps()` lists every installed app with its id and file claims.
...
- `loadExample()` — Show a synthetic 3D tissue

### Backend methods

Heavier work runs in this app's own process: `app_call(app_id="spatial3d", method=..., args={...})`. List the live method signatures with `app_registry()`.
