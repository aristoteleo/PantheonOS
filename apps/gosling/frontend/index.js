...
export async function setup(lv, root) {
  // ── Page chrome, from the host's published palette (docs/app-spec.md).
  // It was semi-transparent whites over an assumed dark panel, which is
  // exactly the thing that cannot survive an appearance setting: on light it
  // left the title white-on-white and the "Whole genome" button invisible.
  // Every colour below is a var the host redefines per theme. ──
  root.style.cssText =
    'width:100%;height:100%;overflow:auto;scrollbar-gutter:stable;box-sizing:border-box;' +
    'color:var(--app-fg);background:transparent;font:13px/1.45 -apple-system,BlinkMacSystemFont,' +
    '"Segoe UI",Roboto,Helvetica,Arial,sans-serif;'
  const card = document.createElement('div')
  card.style.cssText =
    'margin:6px;background:var(--app-raised);border:1px solid var(--app-border);' +
    'border-radius:12px;box-shadow:0 1px 2px rgba(0,0,0,.18);overflow:hidden;box-sizing:border-box;'

  // Sticky top bar = branded header (title) + search toolbar.
  const topbar = document.createElement('div')
  topbar.style.cssText =
    'position:sticky;top:0;z-index:5;background:var(--app-raised);' +
    'border-bottom:1px solid var(--app-border);backdrop-filter:blur(6px);'
  ...
  header.innerHTML =
    ...
    '<div data-role="title" style="font-size:13.5px;font-weight:600;color:var(--app-fg);' +
    'line-height:1.25;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Genome Browser</div>' +
    '<div data-role="subtitle" style="font-size:11px;color:var(--app-dim);line-height:1.3;' +
  ...
  toolbar.innerHTML =
    ...
    'style="flex:1 1 200px;min-width:0;padding:5px 10px;border:1px solid var(--app-border);' +
    'border-radius:8px;font:12.5px ui-monospace,SFMono-Regular,Menlo,monospace;' +
    'color:var(--app-fg);background:var(--app-sunken);outline:none" />' +
    '<datalist id="gos-genes"></datalist>' +
    '<button data-role="go" style="' + btnCss('var(--app-accent)', '#fff') + '">Go</button>' +
    '<button data-role="reset" title="Zoom out to the whole genome" style="' +
    btnCss('var(--app-sunken)', 'var(--app-fg)', 'var(--app-border)') + '">Whole genome</button>' +
  ...
  async function render() {
    ...
    // Gosling ships both; the black canvas under a light desktop was this
  ...
  // The chrome above follows by itself (it is all vars); the canvas does not.
  if (typeof lv.onTheme === 'function') {
    let seen = null
    lv.onTheme((theme) => {
      if (seen !== null && theme !== seen) requestRender()
      seen = theme
    })
  }
