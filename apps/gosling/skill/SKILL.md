...
description: |
  Open and drive a Gosling.js view on the desktop — a grammar-based
...
tags: [gosling, genome, genomics, visualisation, ideogram, multi-track, circular, hi-glass]
---
...
```
desktop_open(app="gosling", title="Gosling Demo")
```
...
- **Local files** — serve them and use the URL:
  ...
  desktop_open(app="gosling", title="...", state={"spec": {
      "tracks": [{
...
- ❌ abandoning gosling to hand-roll a raw HiGlass viewer (a custom desktop app
...
The spec **is** the interface — change it with `desktop_set(window_id,
...
desktop_set(window_id, {"spec": <updated spec>})
```
...
`desktop_read` is the primary check — `status: ready`, empty
`diagnostics`. Diagnostics usually mean a bad spec (unknown field), an
unreachable data URL, or a HiGlass tile that 404'd.

`desktop_screenshot` **works for Gosling** and returns the actual rendered
...
This viewer is installed as the desktop app `gosling` ("Gosling"). Everything above drives it through the `desktop` tools, which reach every window of it — including ones the USER opened. What follows is what is specific to this app.

- **Open by state**: `desktop_open(app="gosling", state={...})` with the state contract documented above.
- **Force this viewer** for a file another app also claims: `desktop_open(app="gosling", path=...)`. `desktop_apps()` lists every installed app with its id and file claims.
