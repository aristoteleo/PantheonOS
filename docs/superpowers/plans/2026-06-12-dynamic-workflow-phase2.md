...
- Create `pantheon/workflow/http.py`，按 chatroom 现有 service/RPC 注册模式（先 grep `proxy_toolset` 与 live_view 的前端调用路径，沿用同一通道）暴露：
  ...
  - `workflow_node_trace(workflow_id, seq)` — 读 nodes/{seq}_{label}.jsonl 返回执行轨迹
  - `workflow_control_ui(workflow_id, action, node_label)` — UI 干预按钮直达 engine.control（与 Leader 工具同一入口，不绕过）
- [ ] 失败测试 → 实现 → 通过 → commit `feat(workflow): add UI query/control endpoints`

### Task 2: 前端类型与 workflowStore
- Create `types/workflow.ts`（字段与后端 events.py 一一对应）与 `stores/workflow.ts`：
  - `applyEvent(event)` 处理 7 类事件（created/node_started/node_finished/phase_changed/log/status/resumed）
