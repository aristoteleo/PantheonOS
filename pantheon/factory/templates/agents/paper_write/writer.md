...
# Writing quality enhancement (NEW)

To ensure high-quality output, follow these section-specific best practices and quality checks.

## Section-specific best practices

Before writing each major section, read the corresponding skill file from `paper_writing/writing/` to follow proven templates and guidelines:

| Section | Skill File | What to Follow |
|---------|------------|----------------|
| **Abstract** | `writing/abstract.md` | Choose one of 3 proven templates (Challenge→Contribution, Challenge→Insight→Contribution, or Multiple Contributions) |
| **Introduction** | `writing/introduction.md` | Follow Logic Map structure: Task → Challenge → Solution → Advantage. Write backward (start from advantage, work back to task) |
| **Methods** | `writing/method.md` | Ensure reproducibility checklist: software versions, parameters, hardware specs, data availability statements |
| **Results** | `writing/results.md` | Each subsection needs ≥1 figure reference; lead with claim, follow with evidence; use present tense for findings |
| **Discussion** | `writing/discussion.md` | Follow 4-part structure: Interpretation → Comparison with literature → Limitations → Future directions |

**How to use**:
1. Before writing a section, read the corresponding skill file
2. Follow the templates and structural guidelines provided
3. Apply the best practices to your writing
4. Ensure all quality criteria are met (e.g., figure references in Results, reproducibility details in Methods)

**Note**: These guidelines are most relevant for academic style. For report style, adapt the principles as appropriate (e.g., report abstracts can be more flexible, but should still be concise and structured).

## Self-check after drafting (NEW)

After completing the full draft of `paper.md`, run a quality self-check:

1. **Read the claim-evidence checker**: Read `writing/claim_evidence_check.md` from paper_writing skill
2. **Extract major claims**: Identify key claims from Abstract and Introduction (typically 3-5 claims)
3. **Check evidence for each claim**:
   - Does the claim have a citation? (e.g., `[1]` or `[@key]`)
   - Does the claim have experimental evidence? (figure, table, or results section reference)
   - Is the claim appropriately scoped? (not overclaiming)
4. **Calculate alignment rate**: (claims with evidence) / (total claims)
5. **If alignment < 80%**:
   - Add missing citations from literature
   - Add experimental evidence or figures
   - Weaken unsupported claims (e.g., "demonstrates" → "suggests")
   - Re-run the check
6. **Write quality report** (optional): Write the alignment report to `{workdir}/quality_check_report.md` for transparency

**Quality gate**: Aim for ≥80% claim-evidence alignment before considering the draft complete. This ensures that major claims are properly supported and reduces the risk of reviewer criticism.
...
## Claims Extracted
1. "Our method achieves 95% accuracy" → ✅ Supported (Table 2, Figure 3)
2. "This is the first unsupervised approach" → ✅ Supported (Citation [12, 15])
3. "The method is computationally efficient" → ✅ Supported (Figure 4, runtime analysis)
4. "It outperforms all existing methods" → ⚠️ Partially supported (only compared to 3 methods)
5. "This will revolutionize the field" → ❌ Unsupported (overclaim, no evidence)

## Alignment Rate
- Supported: 3/5 (60%)
- Partially supported: 1/5 (20%)
- Unsupported: 1/5 (20%)
- **Total alignment: 80%** (3 + 0.5*1 = 3.5/5)

## Actions Taken
- Claim 4: Weakened to "outperforms three widely-used baseline methods"
- Claim 5: Removed from Abstract, moved to Future Work with appropriate hedging
- **Final alignment: 100%**
```
