...
class GenePanelConfig:
    ...
    # --- SpaPROS runtime gate (consumed by the leader's notify_user gate) ---
    spapros_runtime_warning_minutes: float = 5.0
    """Estimated runtime above which the leader must prompt the user for
    approval before running SpaPROS (severity ``"slow"``)."""

    spapros_runtime_skip_minutes: float = 30.0
    """Estimated runtime above which the leader must surface a strong
    warning (severity ``"very_slow"``) — the user may wish to skip
    SpaPROS entirely."""

    # --- Workflow thresholds (consumed by the agent, not the algorithms) ---
