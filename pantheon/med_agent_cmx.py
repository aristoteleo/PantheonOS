...
STRICT RULES:
- Call run_segmentation IMMEDIATELY. Do not ask questions or describe what you will do.
...
STRICT RULES:
- Call run_xray_classification IMMEDIATELY. Do not ask questions or describe what you will do.
- NEVER ask the user for confirmation. NEVER say "would you like to proceed".
...
def build_ct_agent_tool():
    ...
    async def ct_agent_tool(
        ...
        task_str = f"""CALL run_segmentation NOW with these exact parameters:
...
statistics: True
gpu: 1
{"Then call evaluate_segmentation with gt_dir: " + gt_dir if gt_dir else "Then skip evaluation — no gt_dir."}
Then report results.
"""
...
def build_xray_agent_tool():
    ...
    async def xray_agent_tool(
        ...
        task_str = f"""CALL run_xray_classification NOW with these exact parameters:
image_path: {image_path}
output_dir: {output_dir}
model: {model}
threshold: {threshold}
...
def build_router_agent() -> Agent:
    ...
    agent = Agent(
        ...
        icon="🏥",
        instructions="""
You are a medical imaging router. You MUST call exactly one tool — no exceptions.
Responding with only text without calling a tool = failure.

STEP 1 — DETECT MODALITY (in priority order):
1. Explicit "modality:" field in the task → use it, always.
2. File extension in image_path:
   - .png  .jpg  .jpeg  .dcm   → XRAY
   - .nii  .nii.gz             → CT or MRI (check filename)
3. Filename keywords:
   - "CT", "ct_case", "_0000"              → CT
   - "MR", "MRI", "mri", "amos", "T1"     → MRI
4. Still ambiguous → default CT.

STEP 2 — CALL THE TOOL IMMEDIATELY (no text response first):
- CT   → CALL ct_agent_tool()
- MRI  → CALL mri_agent_tool()
- XRAY → CALL xray_agent_tool()

RULES:
- Call exactly one tool. Never call more than one.
- Never describe what you are going to do — just do it.
- Never ask for confirmation.
- Pass gt_dir="" if not provided — never omit it for CT/MRI.

After the tool returns, output:
  Detected modality: <CT | MRI | X-Ray>  (reason: <why>)
  <specialist agent output verbatim>
""",
        model=LOCAL_MODEL,
        tools=[
            build_ct_agent_tool(),
...
# ── Runner — called directly or by demo.py ────────────────────────────────────
...
async def run_router(
    ...
    user_task: str = "",          # ← optional free-text from demo.py NLP parse
) -> None:
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    roi = " ".join(roi_subset) if roi_subset else ""

    # Extension-based pre-detection so the Router gets an unambiguous hint
    ext = Path(image_path).suffix.lower()
    if modality is None and ext in (".png", ".jpg", ".jpeg", ".dcm"):
        modality = "XRAY"

    # If demo.py passed a user_task string, include it as context
    user_task_line = f'\nUser said: "{user_task}"' if user_task else ""

    task_str = f"""CALL THE CORRECT TOOL NOW. Do not respond with text first.{user_task_line}

modality:       {modality.upper() if modality else "auto-detect from image_path"}
image_path:     {image_path}
output_dir:     {output_dir}
...
Detect modality from the modality field or image_path filename, then CALL the tool immediately.
"""
    ...
    step("RouterAgent detecting modality and delegating")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--image", default="datasets/data_exp_ct-mr/CT_case00002/CT_Case_00002_0000.nii.gz")
    parser.add_argument("--output_dir", default="tmp/test_agent_output")
    parser.add_argument("--modality", default=None, choices=["CT", "MRI", "XRAY", "ct", "mri", "xray"])
    parser.add_argument("--roi_subset", nargs="+", default=None)
    parser.add_argument("--fast", action="store_true")
    parser.add_argument("--gt_dir", default=None)
    parser.add_argument("--xray_model", default="densenet121-res224-all",
                        choices=["densenet121-res224-all","densenet121-res224-rsna",
    ...
    parser.add_argument("--xray_threshold", type=float, default=0.5)
