...
class MemoryRuntime:
    ...
    def initialize(self, pantheon_dir: Path, runtime_dir: Path) -> None:
        ...
        self.retriever = MemoryRetriever(
            self.store, model=self.config["selection_model"]
        )
    ...
    async def retrieve_relevant(
        ...
        max_results = self.config.get("max_retrieval_results", 15)
        # Enforce minimum bound only
        max_results = max(1, max_results)
        results = await self.retriever.find_relevant(
            query, max_results=max_results, already_shown=shown
        )
        for r in results:
