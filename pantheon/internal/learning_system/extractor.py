...
class SkillExtractor:
    ...
    async def maybe_extract(
        ...
        # Non-blocking lock acquisition — skip if already in progress
        lock = self._get_lock(session_id)
        if lock.locked():
            logger.info(f"[Learning] Skipping extraction: already in progress (session={session_id})")
            return None
    ...
    def increment_run(self, session_id: str) -> None:
        """Increment run counter for a session."""
        self._run_counter[session_id] = self._run_counter.get(session_id, 0) + 1
        logger.info(f"[Learning] Counter incremented: {self._run_counter[session_id]}/{self.nudge_interval} (session={session_id})")
