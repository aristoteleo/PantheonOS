...
class LearningPlugin(TeamPlugin):
    ...
    async def on_run_start(
        ...
        """Set session context for tool calls during this run."""
        if not self.runtime.is_initialized:
            return
        memory = context.get("memory")
        session_id = getattr(memory, "id", "default") if memory else "default"
        self.runtime._current_session_id = session_id
    ...
    async def on_run_end(self, team: "PantheonTeam", result: dict) -> None:
        ...
        session_id = result.get("chat_id") or self.runtime._current_session_id
